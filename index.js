...
if (fs.existsSync("./config.json")) {
  ...
  if (config.token === "REPLACE_WITH_TOKEN_IN_ENV") {
    config.token = process.env.REVOLT_TOKEN;
  }
} else {
  config = {
    token: process.env.REVOLT_TOKEN || process.env.TOKEN
  };
}
...
class Chatgpt {
  constructor() {
    ...
    this.client.loginBot(config.token).catch(err => {
      console.error(" [Login_Error] :: Failed to login to Revolt!");
      console.error(err);
      process.exit(1);
    });
